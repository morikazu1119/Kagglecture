...
summary: GBDT、CNN、Transformer、Tabular DL、Multimodal、GNNをモデル単位で整理する。
type: category-index
nav_order: 3
...
Kagglectureでは、原則として**1つのモデル / architecture familyを1記事**にします。CNN記事の中でResNet・EfficientNet・ConvNeXtをまとめて覚えるのではなく、まず共通原理を理解し、その後それぞれのモデル固有設計を個別に確認できます。
...
<div class="comparison-board" aria-label="Kaggleで使われる主要モデルfamily">
  ...
  <section class="comparison-card is-primary"><h4>Vision</h4><dl><dt>主な入力</dt><dd>画像 / spectrogram</dd><dt>核</dt><dd>Convolution / Attention</dd><dt>代表</dt><dd>ResNet / ConvNeXt / Swin / U-Net / YOLO</dd></dl></section>
  <section class="comparison-card"><h4>Text / Sequence</h4><dl><dt>主な入力</dt><dd>token / 時系列</dd><dt>核</dt><dd>Attention / recurrent state / SSM</dd><dt>代表</dt><dd>BERT / DeBERTa / GRU / Mamba</dd></dl></section>
  <section class="comparison-card"><h4>Tabular DL</h4><dl><dt>主な入力</dt><dd>表形式</dd><dt>核</dt><dd>feature embedding / MLP / Attention</dd><dt>代表</dt><dd>FT-Transformer / TabPFN / RealMLP / TabM</dd></dl></section>
...
<div class="dictionary-grid">
  ...
  <a class="dictionary-card dictionary-card-link" href="{{ '/wiki/modeling/roberta.html' | relative_url }}"><h3>RoBERTa</h3><p>BERTのtraining recipeを再設計し、より強いencoderを作る。</p></a>
  ...
  <a class="dictionary-card dictionary-card-link" href="{{ '/wiki/modeling/mamba.html' | relative_url }}"><h3>Mamba</h3><p>Selective State Spaceで必要な情報を選択しながらsequenceを線形時間で走査する。</p></a>
</div>
...
<div class="dictionary-grid">
  ...
  <a class="dictionary-card dictionary-card-link" href="{{ '/wiki/modeling/realmlp.html' | relative_url }}"><h3>RealMLP</h3><p>tabular向け前処理・numeric embedding・training defaultsを詰めた強いMLP。</p></a>
...
各記事は、**1サンプルで何が起きるか → モデル全体でどう積み上がるか**の2段階で説明します。モデル名だけではなく、input shape、主要演算、intermediate representation、connection、最終outputを図で追うのがこのカテゴリの基準です。
